...
{
    //获取launch文件中的参数，读取配置
    // src/ros-bridge/carla_shenlan_projects/carla_shenlan_lqr_pid_controller/config/lqr_parameters_configuration.yaml
    ...
    lqr_controller_lateral->LoadControlConf();//加载配置
    lqr_controller_lateral->Init();//初始化A、B、K、Q、R矩阵的系数

    //订阅车辆坐标，ego_vehicle是自车名称，在下列模块中定义
    // src/ros-bridge/carla_shenlan_projects/carla_shenlan_bridge_ego_vis/launch/command_control.launch.py
    ...
    //定时执行，车辆控制回调
    vehicle_control_iteration_timer = this->create_wall_timer(10ms,
                                                              std::bind(&LQRControllerNode::VehicleControllerIterationCallback, this));

    //定时执行，全局路径发布器
    global_path_publish_timer = this->create_wall_timer(500ms,
                                                        std::bind(&LQRControllerNode::GlobalPathPublishCallback, this));
...
**************************************************************************************'''*/
{
    ...
    vehicleState_.velocity = std::sqrt(vehicleState_.vx * vehicleState_.vx + vehicleState_.vy * vehicleState_.vy
                                       + vehicleState_.vz * vehicleState_.vz) * 3.6;    // 本车速度
...
/*'''**************************************************************************************
- FunctionName: 回调IMU传感器
- Function    : None
- Inputs      : None
- Outputs     : None
- Comments    : 测量车辆的加速度，转动角速度
**************************************************************************************'''*/
{
...
**************************************************************************************'''*/
{
    ...
    while (getline(infile, _line)) {
        ...
        //按照逗号分隔
        //85,0.001,324.328,-195.387,0.6721257322852878
    ...
    for (size_t i = 0; i < headings.size(); i++) {
        ...
        std::cout << " kappa: " << kappas[i] << " dkappas: " << dkappas[i] << std::endl;
    }
...
**************************************************************************************'''*/
{
    ...
    if (!firstRecord_) {    //有定位数据开始控制
        ...
        //LQR横向控制量
        if (!isReachGoal_) {
            lqr_controller_lateral->ComputeControlCommand(this->vehicleState_,
                                                          this->planning_published_trajectory, cmd);
        }
        ...
        cout << "方向盘转角：" << control_cmd.steer * 180 / M_PI << ", 油门：" << control_cmd.throttle;
        cout << ", 刹车：" << control_cmd.brake << endl;
    }
}
